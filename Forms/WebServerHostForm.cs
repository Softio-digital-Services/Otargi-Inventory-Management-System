...
{
    /// <summary>
    /// Borderless desktop host embedding the Otargi web UI.
    ...
    public class WebServerHostForm : Form
    {
        ...
        private static readonly Color BrandColor = Color.FromArgb(16, 185, 129);
        ...
        private async Task InitializeAsync()
        {
            try
            {
                ...
                if (!await readyTask)
                {
                    ShowFatal("Otargi could not start the local service.\nPlease restart the application.");
                    return;
                }
                ...
                if (!ok)
                {
                    ShowFatal("Otargi failed to load the interface.\nPlease restart the application.");
                    return;
                }
            ...
            {
                ShowFatal("Otargi failed to open:\n" + ex.Message);
            }
        }
        ...
        private void HandlePrintBarcodes(string json)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(this, "Could not print labels:\n" + ex.Message,
                    "Otargi Inventory", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void ShowFatal(string message)
        {
            Opacity = 1;
            _splash.Visible = false;
            MessageBox.Show(this, message, "Otargi Inventory", MessageBoxButtons.OK, MessageBoxIcon.Error);
            Close();
        }
