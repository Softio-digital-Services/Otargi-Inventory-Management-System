# Otargi Inventory Management System
...
### Run published web app

```text
dist\app\OtargiInventorySystem.exe
```

This opens the Otargi web UI (dashboard, inventory, POS, reports, …). Hardware scale / sell-by-weight can be enabled by Softio Super Admin in Settings.
...
```text
bin\Debug\net8.0-windows\win-x64\OtargiInventorySystem.exe
```
...
3. **Database** — created automatically under `%LocalAppData%\OtargiInventory\Data\inventory.db` (no SQL Server needed)
...
├── Assets/                  ← Icons / branding (logo.png)
├── Templates/               ← CSV import templates
├── installer/               ← Inno Setup script + build.ps1
├── publish.bat              ← Publish app + OtargiSetup.exe
...
### Database
SQLite file lives in `%LocalAppData%\OtargiInventory\Data\inventory.db` so the app can save data even when installed under Program Files.
...
### Theme Customization
Edit `Helpers\ThemeConfig.cs` (Otargi green: `#10B981`).
...
Backups and the live database use `%LocalAppData%\OtargiInventory\`:
